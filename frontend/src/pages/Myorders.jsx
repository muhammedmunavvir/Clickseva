...
export const Myorders = () => {
  ...
  return (
    <div className="p-6 min-h-screen bg-gradient-to-br from-gray-100 to-white">
      <h2 className="text-4xl font-bold mb-8 text-center text-indigo-700">My Orders</h2>
          ...
          {myorders.map((order, index) => (
            ...
            >
              ...
              <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 border-b pb-4">
                ...
                <div><button className="bg-red-500 rounded-md p-1 text-white"
                 disabled={order.status === "Cancelled"}
                 onClick={()=>ordercancel(order.orderId)}>cancel order</button></div>
              ...
              <div className="mb-4 text-gray-700">
                ...
                <p>{order.shippingAddress.address}, {order.shippingAddress.housename}</p>
                <p>{order.shippingAddress.place}, {order.shippingAddress.postOffice}</p>
              ...
              <div className="mb-6 text-gray-700">
                ...
                <p>Total: <span className="font-bold text-indigo-600">₹{order.totalAmount}</span></p>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 overflow-x-auto">
                  {order.products.map((product, i) => (
                    ...
                    >
                      ...
                      />
                      <h4 className="text-md font-bold text-gray-800">{product.heading}</h4>
                      <p className="text-sm text-gray-600 line-clamp-2">{product.discription}</p>
                      <p className="mt-1 text-sm text-gray-500">Category: {product.catogory}</p>
                      <p className="mt-1 text-md font-semibold text-indigo-700">₹{product.price}</p>
